...
export function OutputTabs({ result }: Props) {
  ...
  if (!result) {
    return (
      <section className="rounded-3xl border border-dashed border-slate-300 bg-white/70 p-6 text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-400">
              ...
              <ul className="grid gap-2 md:grid-cols-2">
                {result.grammarAnalysis.partsOfSpeech.map((part) => (
                  <li
                    ...
                    className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm dark:border-slate-700 dark:bg-slate-800"
                  >
